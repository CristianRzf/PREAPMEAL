...
async function main() {
  ...
  console.log(`Snacks ya en Firestore: ${idsExistentes.size}`);
  ...
  const total = await db.collection("recipes").where("mealType", "==", "snack").get();
  console.log(`\nCompletado - Nuevas: ${exitosas} / Duplicadas: ${duplicadas} / Fallidas: ${fallidas}`);
  console.log(`Total snacks en Firestore: ${total.size}`);
  process.exit(0);
}
