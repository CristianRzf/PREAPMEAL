...
export default function Perfil() {
  ...
  useEffect(() => {
    if (!user) return;
    const unsubFollowers = onSnapshot(collection(db, "users", user.uid, "followers"), (snap) => setSeguidores(snap.size));
    const unsubFollowing = onSnapshot(collection(db, "users", user.uid, "following"), (snap) => setSiguiendo(snap.size));
  ...
  const fetchUser = async () => {
    ...
    finally { setLoading(false); cargarMisRecetas(); }
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        {/* Avatar — tap abre modal blur */}
        <View style={styles.profileSection}>
          <TouchableOpacity
            onPress={() => fotoPerfil && setFotoModalVisible(true)}
        ...
        {activeTab === "Guardadas" && (
          loadingSaved ? (
            ...
            <View style={styles.miniGrid}>
              {savedRecetas.map((receta) => <RecetaMiniCard key={receta.id} receta={receta} />)}
            </View>
          )
        )}
        ...
        {activeTab === "Me gusta" && (
          loadingLiked ? (
            ...
            <View style={styles.miniGrid}>
              {likedRecetas.map((receta) => <RecetaMiniCard key={receta.id} receta={receta} />)}
            </View>
          )
        )}
