...
function calcularTDEE(
  ...
  if (!edad || !peso || !altura || !sexo || !actividad || !objetivo)
    return null;
  const tmb =
    sexo === "Masculino"
  ...
  const tdee = Math.round(
    tmb * (ACTIVIDAD_FACTORES[actividad] ?? 1.2) +
      (OBJETIVO_AJUSTE[objetivo] ?? 0),
  );
...
function RecetaMiniCard({ receta }: { receta: RecetaItem }) {
  return (
    <View style={miniStyles.container}>
      ...
      <Text style={miniStyles.titulo} numberOfLines={2}>
        {receta.titulo}
      </Text>
      {receta.username && (
        <Text style={miniStyles.autor}>@{receta.username}</Text>
      )}
    </View>
  );
}
...
export default function Perfil() {
  ...
  const cargarMisRecetas = async () => {
    ...
    try {
      ...
      setMisRecetas(
        snap.docs.map((d) => ({ id: d.id, ...d.data() }) as RecetaItem),
      );
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const guardar = async () => {
    ...
    if (uploadingFoto) {
      Alert.alert("Espera", "La foto aún se está subiendo.");
      return;
    }
    const displayName = form.displayName.trim();
    const username = sanitizeUsername(form.username);
    const edad = Number(onlyDigits(form.edad));
    const peso = Number(sanitizeDecimal(form.peso, FORM_LIMITS.weight));
    const altura = Number(onlyDigits(form.altura));

    if (!displayName || displayName.length < 2) {
      Alert.alert("Error", "El nombre debe tener al menos 2 caracteres.");
      return;
    }
    if (displayName.length > FORM_LIMITS.displayName) {
      Alert.alert(
        "Error",
        `El nombre no puede superar ${FORM_LIMITS.displayName} caracteres.`,
      );
      return;
    }
    ...
    if (username.length > FORM_LIMITS.username) {
      Alert.alert(
        "Error",
        `El usuario no puede superar ${FORM_LIMITS.username} caracteres.`,
      );
      return;
    }
    ...
    if (Number.isNaN(peso) || peso < 1 || peso > 400) {
      Alert.alert("Error", "Ingresa un peso válido.");
      return;
    }
    if (Number.isNaN(altura) || altura < 30 || altura > 300) {
    ...
    try {
      await setDoc(
        ...
        {
          ...form,
          displayName: displayName.slice(0, FORM_LIMITS.displayName),
          username,
          edad,
          peso,
          altura,
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
            ...
            {userData.macros && (
              <View style={styles.macrosRow}>
                <View style={styles.macroItem}>
                  <View
                    style={[styles.macroDot, { backgroundColor: "#4A90E2" }]}
                  />
                  <Text style={styles.macroLabel}>Proteínas</Text>
                  <Text style={styles.macroValue}>
                    {userData.macros.proteinas}g
                  </Text>
                </View>
                <View style={styles.macroItem}>
                  <View
                    style={[styles.macroDot, { backgroundColor: "#F5A623" }]}
                  />
                  <Text style={styles.macroLabel}>Grasas</Text>
                  <Text style={styles.macroValue}>
                    {userData.macros.grasas}g
                  </Text>
                </View>
                <View style={styles.macroItem}>
                  <View
                    style={[styles.macroDot, { backgroundColor: "#7ED321" }]}
                  />
                  <Text style={styles.macroLabel}>Carbos</Text>
                  <Text style={styles.macroValue}>
                    {userData.macros.carbohidratos}g
                  </Text>
                </View>
              </View>
            )}
        ...
        {/* Alergias */}
        {userData?.alergias &&
          Object.values(userData.alergias).some(Boolean) && (
              ...
              {misRecetas.map((receta) => (
                ...
                >
                  {receta.imagen ? (
                    ...
                    <View
                      style={[styles.gridImage, styles.gridImagePlaceholder]}
                    >
                      <Ionicons
                        name="restaurant-outline"
                        size={24}
          ...
          (loadingSaved ? (
            ...
            <View style={styles.emptyGrid}>
              ...
              <Text style={styles.emptySubText}>
                Toca 🔖 en cualquier receta para guardarla
              </Text>
            </View>
          ) : (
          ...
          (loadingLiked ? (
            ...
            <View style={styles.emptyGrid}>
              ...
              <Text style={styles.emptyText}>
                No has dado me gusta a ninguna receta
              </Text>
              <Text style={styles.emptySubText}>
                Toca ❤️ en cualquier receta para darle me gusta
              </Text>
            </View>
          ) : (
            <View style={styles.miniGrid}>
          ...
          >
            <View style={styles.modalAvatarSection}>
              ...
              >
                {fotoPerfilLocal || form.fotoPerfil ? (
                  <Image
                    source={{ uri: fotoPerfilLocal || form.fotoPerfil }}
              ...
              <Text style={styles.changePhoto}>
                {uploadingFoto ? "Subiendo foto..." : "Toca para cambiar foto"}
              </Text>
            </View>
            ...
            <TextInput
              ...
              onChangeText={(v) =>
                setForm({
                  ...form,
                  displayName: limitText(v, FORM_LIMITS.displayName),
                })
              }
              placeholder="Tu nombre completo"
              placeholderTextColor={COLORS.textMuted}
              maxLength={FORM_LIMITS.displayName}
            />
            ...
            <View style={styles.usernameInputRow}>
              ...
              <TextInput
                ...
                value={form.username}
                onChangeText={(v) =>
                  setForm({ ...form, username: sanitizeUsername(v) })
                }
              ...
              <View style={{ flex: 1 }}>
                ...
                <TextInput
                  ...
                  onChangeText={(v) =>
                    setForm({
                      ...form,
                      edad: onlyDigits(v).slice(0, FORM_LIMITS.age),
                    })
                  }
                  placeholder="años"
                  keyboardType="numeric"
                  placeholderTextColor={COLORS.textMuted}
                  maxLength={FORM_LIMITS.age}
                />
              </View>
              <View style={{ flex: 1 }}>
                ...
                <TextInput
                  ...
                  onChangeText={(v) =>
                    setForm({
                      ...form,
                      peso: sanitizeDecimal(v, FORM_LIMITS.weight),
                    })
                  }
                  placeholder="kg"
                  keyboardType="numeric"
                  placeholderTextColor={COLORS.textMuted}
                  maxLength={FORM_LIMITS.weight}
                />
              </View>
            </View>
              ...
              <View style={{ flex: 1 }}>
                ...
                <TextInput
                  ...
                  onChangeText={(v) =>
                    setForm({
                      ...form,
                      altura: onlyDigits(v).slice(0, FORM_LIMITS.height),
                    })
                  }
                  placeholder="cm"
                  keyboardType="numeric"
                  placeholderTextColor={COLORS.textMuted}
                  maxLength={FORM_LIMITS.height}
                />
              </View>
            </View>
              ...
              <View style={styles.calcCard}>
                ...
                <View style={styles.macrosRow}>
                  {[
                    {
                      label: "Proteínas",
                      value: calculo.proteinas,
                      color: "#4A90E2",
                    },
                    {
                      label: "Grasas",
                      value: calculo.grasas,
                      color: "#F5A623",
                    },
                    {
                      label: "Carbos",
                      value: calculo.carbohidratos,
                      color: "#7ED321",
                    },
                  ].map((m) => (
            ...
            <View style={styles.alergiasGridForm}>
              {ALERGIAS_LISTA.map((alergia) => (
                <TouchableOpacity
                  key={alergia}
                  style={[
                    styles.alergiaChipForm,
                    form.alergias[alergia] && styles.alergiaChipFormActive,
                  ]}
                  onPress={() =>
                    setForm({
                      ...form,
                ...
                >
                  <Text
                    style={[
                      styles.alergiaChipText,
                      form.alergias[alergia] && styles.alergiaChipTextActive,
                    ]}
                  >
            ...
            <TouchableOpacity
              style={[
                styles.saveButton,
                (saving || uploadingFoto) && { opacity: 0.7 },
              ]}
              onPress={guardar}
              disabled={saving || uploadingFoto}
            >
