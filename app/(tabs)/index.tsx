...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const unsubInv = onSnapshot(collection(db, "users", user.uid, "pantry_inventory"), (snap) => {
    ...
    const unsubRec = onSnapshot(collection(db, "users", user.uid, "recipes"), (snap) => setRecipesCount(snap.size));

    const unsubProfile = onSnapshot(doc(db, "users", user.uid, "profile", "nutricional"), (snap) => {
    ...
    const unsubBudget = onSnapshot(doc(db, "users", user.uid, "budget", "config"), (snap) => {
    ...
    const unsubTx = onSnapshot(
      ...
      (snap) => {
        setTransactions(snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<Transaction, "id">) })).filter((t) => t.fecha?.toDate?.() >= startOfMonth));
      }
    );
  ...
  const gestureHandler = Gesture.Pan()
    ...
    .onEnd((event) => {
      ...
      if (Math.abs(event.velocityX) > 500 || Math.abs(event.translationX) > threshold) {
        const newIndex = event.translationX > 0
  ...
  const totals = meals.reduce(
    (acc, m) => ({ kcal: acc.kcal + (m.kcal ?? 0), proteina: acc.proteina + (m.proteina ?? 0), carbos: acc.carbos + (m.carbos ?? 0), grasas: acc.grasas + (m.grasas ?? 0) }),
