...
async function main() {
  ...
  console.log(`Desayunos ya en Firestore: ${idsExistentes.size}`);
  ...
  const total = await db.collection("recipes").where("mealType", "==", "desayuno").get();
  console.log(`\nCompletado - Nuevas: ${exitosas} / Duplicadas: ${duplicadas} / Fallidas: ${fallidas}`);
  console.log(`Total desayunos en Firestore: ${total.size}`);
  process.exit(0);
}
