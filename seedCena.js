...
async function main() {
  ...
  console.log(`Cenas ya en Firestore: ${idsExistentes.size}`);
  ...
  const total = await db.collection("recipes").where("mealType", "==", "cena").get();
  console.log(`\nCompletado - Nuevas: ${exitosas} / Duplicadas: ${duplicadas} / Fallidas: ${fallidas}`);
  console.log(`Total cenas en Firestore: ${total.size}`);
  process.exit(0);
}
